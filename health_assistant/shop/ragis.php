<?php
	...
	if(isset($Ragistraion))
	{
		...
		echo "hi";
		$q="insert into shop(usernm,pass,sknm,snm,phone,certi,address,status) values('$unm','$pass','$nm','$snm','$ph','$file_name','$location',1)";
		if(mysqli_query($conn,$q))
		{
			...
			if (mysqli_num_rows($res) > 0) 
			{
				while($row = mysqli_fetch_assoc($res)) 
				{
					@session_start();
					$id=$row['id'];
					echo "<script>alert(your id will be $id);</script>";
					$_SESSION['sid1']=$row["id"];
				}
			}	
...
{	
	...
	display:inline-block;
			...
			function check()
			{
				...
				if(document.frm1.location.value=="")
				{
						document.getElementById("locate1").innerHTML = "Location ".fontcolor("red") + result;
						i++;
				}
				if(document.frm1.certi.value=="")
				{
					...
					<div class="c">
						<table cellpadding="3px" cellspacing="3px">
							...
							<tr>
								<td>Shop Location : 
								<td>
									<input type="textbox" name="location" id="location" placeholder="" /><i class="fa fa-map-marker"></i><font color="red">*</font>
								<td><h4 id="locate1"></h4>
							</tr>
							<tr>
